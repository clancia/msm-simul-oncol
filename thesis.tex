...
estimate of the population parameter \(\hat \theta_1\). The same
procedure applied to a second sample \(X_2\) drawn from the same
population will result in second estimate \(\hat \theta_2\), and so on
for more samples. Because only one sample is tyically available, the
sampling distribution of \(\hat \theta_i\) is used to make inferences.
\linebreak 
...
In the example of ibuprofen on headaches it was reasonable to ask
whether the effect of ibuprofen \(X\) on headache alleviation \(Y\) was
different amongst men and women. An effect measure like a rate
difference, rate ratio or odds ratio is collapsible when the effect is
equal in both strata and equal to marginal (over \(C\)) effect.
Collapsibility is a useful property because it means that analysis can
be carried out on a subset of variables after marginalizing over the
others \citet{Didelez2010}. In this case that could mean marginalizing
over sex and looking at just the relationship between \(Y\) and \(X\).
\linebreak  
...
Not collapsible when effect measure in two strata is equal but not equal
to marginal.

\begin{itemize}
\tightlist
\item
  Why is this a problem generally or something that should be accounted
  for by researchers
\item
  Why is this a problem in this case.
\item
  show that it is hard to get a specific MSM.
\end{itemize}
...
Even when the odds ratio is the same over all values of \(C\) it is not
always collapsible in the sense that the effect (measured by odds ratio)
is the same in group \(C\) and group \(C'\)
...
can be explained using a simpliefies two period version of the algorith,
