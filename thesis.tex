...
models and introduces the notation that will be used in this thesis.
...
comparison is now between \(\mathbb{P}(Y_{a = 1})\) and
\(\mathbb{P}(Y_{a = 0})\), the latter being the counterfactual had none
of sixty subjects been exposed. We do not observe the quantity
\(Y_{a=0}\) for any subject, and consequently we do not observe the
quantity \(\mathbb{P}(Y_{a = 0})\). Instead, in both experimental and
...
whether the subject actually received treatment or not. For example, if
every member of group A was given a glass of water and not in group B,
then In which case \(\mu_{A=1} - \mu_{B=0}\), a measure of association,
is confounded for \(\mu_{A=1} - \mu_{A=0}\), a measure of effect
(\citet{Greenland1999}). For instance, if the first thirty subjects who
make up group A are all female and the last thirty subjects that make up
group B are all male, we might expect the probability of having a
headache to differ between the two groups regardless of the treatment
they receive. In this case \(\mu_{A=1} - \mu_{B=0}\) is confounded for
\(\mu_{A=1} - \mu_{A=0}\) by sex. Subjects are not exchangeable because
the distribution of their potential outcomes is not independent of the
treatment they receive. A subject from group B cannot be viewed as an
analogue of a subject from group A under different treatment.
Exchangeability implies that there are no unmeasured
confounders.\linebreak
...
one another. The direction of causal relationships is denoted by an
...
different points in time. The right hand side of figure ? gives an
example of this for two time periods (\(t = 0, 1\)). In the simplest
situation time dependent confounding means that the confouding variables
change in value over time. This is represented in figure ? where the
confounding variable \(L\) takes the value \(L_0\) in the first period
and \(L_1\) in the second period.
...
\end{tikzpicture}
\caption{Causal graph2}
\end{figure}

    What sets the where a subject receives a treatment decision \(A\) in two
time periods, \(L\) At each time point \(t\) in a clinical trial, a
subject may be prescribed treatment (\(A_{t}=1\)) or not (\(A_{t}=0\)).
The treatment history of the subject at any point \(t\) is represented .
Time dependent confounders also change value over time. This opwhich
change their value

In this context confounding variables can also be time dependent meaning
that their value changes over time. Just as in the time fixed case this
can lead to biased results and needs to be adjusted for. What
distinguishes the time dependent confounding case is that the confounder
may itself be a function of previous treatment decisions. Measurments
are typically taken at baseline and subsequent measurements are taken
Just as in the time fixed case, confounding variables prevent causal
analysis and need to be adjusted for. In the time dependent setting
confounders can be fixed meaning that they have the same value in each
time period, or they can be time dependent meaning that their value
changes over time. The time dependen
...
confounder. However, the confounder may be a function of past treatment
and may themselves be determinants of future treatments
(\citet{Robins2000a}).
...
Time dependent data involves observations on the same subject at
multiple points in time. For example, in a clinical trial individual
subjects are measured at baseline and at subsequent points in time.
Similar to the time fixed case, confounding variables can exist which
prevent causal analysis. Unlike the time fixed case, it is not possible
to estimate the causal effect of a treatment on an outcome of interest
because time dependent confounding factors will be both determinants of
later treatment and will themselves be affected by earlier treatment .
In this section we describe several important characteristics of time
dependent confounding using directed acyclic graphs. For example, figure
? represents the time fixed case where a treatment variable \(A\) shows
a simple relationship between three variables \(A\), \(L\) and \(Y\).
represents the structural relationship between The resulting directed
acyclic graph (DAG) \(G = (\nu, \epsilon)\) shown in figure 1 can be
represeted as a set of connections \({(A, Y), (L, Y)}\) where direction
is from the first element of each pair to the second element.

Show the case where the current confounder is independent of past
treatment.
