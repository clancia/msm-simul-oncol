...
ibuprofen on headache alleviation was confounded by sex. To broaden the
setting to a time dependent context, we adopt the canonical example of
the causal effect of Zidovudine (AZT) on mortality amongst human
immunodeficiency virus (HIV)-infected subjects \citet{Hernan2000}.
Subjects are followed over multiple visits and in each each visit a
treatment decision is made and CD4 count is measured. Survival at the
end of follow-up is a binary outcome equal to 1 if the patient has died
and 0 otherwise. \linebreak

The time-fixed notation is extended to include subject histories of
treatment decision and covariates at each visit. The vector of treatment
histories is denoted with an overhead bar. For example, treatment
decisions up to visit \(k\) are denoted
\(\bar X_{k} = \{X_1, \hdots, X_k\}\). Similarly, covariate histories
are denoted \(\bar L_{k} = \{L_1, \hdots, \L_k\}\). Covariates like sex
may be confounders but do not change over time. Such covariates are
tyically recorded at baseline (\(t = 0\)) and we denote the collection
of baseline covariates as \(V_{0}\). The outcome of interest at the end
of follow-up is mortality \(Y\) which is a binary variable taking the
value \(1\) if the patient is dead and \(0\) otherwise. \linebreak 

In many studies subjects are followed over time periods and their
outcome is measured at the end of follow-up. This type of analysis falls
under survival analysis. Between the start of follow up (\(t = 0\)) and
the end of follow-up (\(t = T\)) the treatment . Under this data
structure further considerations must be taken to isolate the causal
effect of \(X\) on \(Y\) in addition to confounding in the time-fixed
context. and both the value of the treatment and any confounding
variables can change over time. The outcome is defined at the end of the
follow up period. \linebreak
