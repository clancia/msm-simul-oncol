...
violations in their data. \linebreak
...
The simulations carried out in this thesis use the Python programming
language.
...
the core parts of MSMs needed for this study - linking to positivity.
    ...
    \subsubsection{Counterfactuals and
...
quantity \(\mathbb{P}(Y_{x = 0})\).

From one point of view causal problems are missing data problems
(\citet{Edwards2015}, \citet{Howe2015}, \citet{}) Instead, in both
experimental and observational settings, subjects exposed to treatment
are typically compared to subjects who are unexposed to treatment.
\linebreak

\subsubsection{Confounding}\label{confounding}

For example, if the first thirty headache sufferers are placed in group
A and are exposed to ibuprofen we would ideally compare the quantity
\(\mathbb{P}(Y_{x = 1}|X = 1) = \mu_{A_{x = 1}}\) with the quantity
\(\mathbb{P}(Y_{x = 0}|X = 1) = \mu_{A_{x = 0}}\). Notice a slight
redundancy in notation. Subjects in group A received treatment and it
makes sense to write \(\mathbb{P}(Y_{x = 0}|X = 1)\) because it
represents a counterfactual; the counterfactual situation that the
subject did not receive treatment \(Y_{x = 0}\) and yet belonged to
group A (\(X = 1\)). In other words, conditioning on \(X = 1\)
highlights the group to which the subject belongs and the redundancy is
to also use \(A\) to reflect the group. As
\(\mathbb{P}(Y_{x = 0}|X = 1)\) is unobserved, we could instead compute
the observable quantity \(\mathbb{P}(Y_{x = 0}|X = 0) = \mu_{B_{x=0}}\)
from the last thirty subjects who were not exposed to treatment and
belong to group B. \linebreak

Replacing the comparison between \(\mu_{A_{x = 1}}\) and
\(\mu_{A_{x = 0}}\) with the comparison between \(\mu_{A_{x = 1}}\) and
\(\mu_{B_{x=0}}\) will have a causal interpretation if
\(\mu_{A_{x = 0}} = \mu_{B_{x=0}}\). If
\(\mu_{A_{x = 1}} \neq \mu_{B_{x=0}}\) then membership of group \(A\)
alters the probability of having a headache even in the counterfactual
case that the the subjects from group A are unexposed. In other words
being a member of group A affects the probability that \(Y = 1\)
regardless of whether the subject actually received treatment or not.
For example, dehydration also causes headaches. If every member of group
A received a glass of water with the ibuprofen then it may be the water
that alleviates the headache or the ibuprofen or both. In this case
\(\mu_{A_{x = 1}} - \mu_{B_{x=0}}\), a measure of association, is
confounded for \(\mu_{A_{x = 1}} - \mu_{A_{x = 0}}\), a measure of
causal effect (\citet{Greenland1999}). \linebreak

Subjects are not exchangeable because the distribution of their
...
are no unmeasured confounders.\linebreak

Explain how we condition on or control for confounding. Also give
intuition for why it works within strata. The cause of within strata
variability cannot be the strata itself. For example, the residual
confoudning after controlling for sex cannot be the result if sex if all
members of that strata are female. Hint at the idea of ever finer strata
to set up positivity as a trade off with exchangeability. Lack of
adjustment for a confounder will lead to bias but adjusting on a
confounder will lead to .... selection bias!

The result of confounding is that the causal effect is mixed with the
confounding. In other words, L may be the cause of A and Y or the cause
may be mixed between A and L

In other words, holding constant that every member of group A will drink
a glass of water regardless of the treatment they receive, it would not
be appropriate to compare these groups.

Use the example that being female cannot be the cause of a difference
between treated and nt treated if we make the comparison in the strata
of all female. Fundementally we want to make a comparison between
treated and untreated groups and be able to resolve the correct causal
comparison in the population. We do this by comparing treated and
untreated subjects within the same strata. For example, if both treated
and untreated patients recived a glass of water then the difference in
outcome between the two cannot be due to receiving a glass of water. The
back door path allows a spurious association between A and Y not the
causal effect we are trying to quantify.
...
For example, the left hand diagram of figure ? represents the time fixed
case where interest lies in the causal relationship between treatment
\(A\) and outcome \(Y\) but the relationship is confounded by \(L\).

Graphs can also be used to illustrate adjustment and conditioning. For
example, conditioning on L can be represented graphically by blocking
the back door path from A to Y. there is a path from A to Y other than
the causal one. Even in the situation, represented graphically in figure
1 where there is only one causal arrow encoding a causal relationship
between A and Y.
...
\end{figure}
    ...
    \subsubsection{Time dependent confounding (500
words)}\label{time-dependent-confounding-500-words}

In longitudinal studies measurements are taken on the same subject at
different points in time. Interest is still in the causal effect of a
treatment of outcome and confounding prevents causal analysis. In the
time dependent case confounders change value over time, as illustrated
in the right hand pane of figure ? for two time periods (\(t = 0, 1\)).
In the first period the relationship between \(A_0\) and \(Y\) is
confounded by \(L_0\) which is a cause of both \(A_0\) and \(Y\). In the
second period, the causal relationship between \(A_1\) and \(Y\) is
confounded for \(L_1\) which is a cause of both \(A_1\) and \(Y\). In
addition, \(L_0\) is a cause of \(A_1\) and \(Y\) and so the
relationship is confounded for both L\_0 and L\_1. Despite the more
complicated structure, adjustment on L\_0 ad L1 will provide consistent
estimated of the causal effect of \(A\) on \(Y\).

Explain why it is clear that adjustment is needed on L because spurious
associations will still exist and then explain that this is fine in one
situation but not in another.

Just as in the time fixed case, adjustment on L\_0 and L\_1 will provide
consisent estimates of the causal relationship because. We can adjust
for the confounder by stratifying on it or adjusting for it.
Intuitively, the cause of the difference cannot be due to the strata if
we make the comparison within the strata. Graphically, the action of
conditoning is the same as blocking the path but this is just a
graphical representation, the blocking actually just means asjusting or
stratifying for.

The right hand pane of figure ? gives an example of this for two time
periods ). At its simplest, time dependent confounding means that the
value of the confounder \(L\) changes over time. In figure ?, \(L\)
takes the value \(L_0\) at time \(t = 0\) and \(L_1\) at time \(t = 1\).
Primary interest is in the relationship between \(A\) and \(Y\).
Analogous to the time fixed situation, \(L\) is a confounder for \(A\)
because it is a determinant of \(A\) and \(Y\). Adjusting for the
covariate distorts the estimate by blocking the path between \linebreak

Fundementally the problem is still that confounders prevent causal
analysis. If we adjust for them then we get

The right hand pane shows a different situation. At \(t = 1\), \(L_1\)
is a function of \(A_0\). In other words, in the time dependent case it
is possible that the confounder is a function of past treatment and a
determinant of future treatments (\citet{Robins2000a}). In comparison,
the right hand side of figure ? has no edge from \(A_0\) to \(L_1\). In
other words the path between A\_0 and Y is mediated by \(L_1\)
distorting the relationship between A\_0 and \(Y\). \linebreak

What Havercroft calls mediation is just the case where there is a path
from A\_0 to L\_1 and on to Y The second problem is selection bias which
is the same in Daniels, havercroft and the discussion by greenland on
common causes.

A second problem is selection bias. Conditioning on L will create an
association between A0 and Y even in the absence of an actual
relationship. This is because L is a common cause of A and Y. Daniel
etal says that ths simple case of the time dependent setting will lead
to a good estimate so there must be some resolve between harvercroft and
Daniel. Explain why in the selection bias case, the fact that U is a
common cause does not mean that it is an unmeasured confounder.

Both the treatment decision \(A\) and the time dependent confounder
\(L\) change over time. Their histories can be represented by the vector
of all treatment decisions up to that point and denoted with an overhead
bar \(\bar A_{t} = \{A_{1}, \hdots, A_{t-1}\}\).

Conclusion, the complicated structure of time dependent data is what we
will need to simulate from.
...
\end{minipage}
\hspace{3cm}% NO SPACE BETWEEN \end \hspace and \begin!
\begin{minipage}{.2\textwidth}
...
\end{figure}
    ...
    In many applications measurements are made on subjects at multiple
points in time. Time dependent confounding differs from the time fixed
case in that the value of the confounder can change over time. Just as
in the time fixed case bias arises when adjustment is not made for the
confounder. However, .

Set up the longitudinal/time dependent case.

Explain why time dependent confounding is more difficult to deal with

Define/explain what time dependent confounding is

Give grahical representation

Explain the problems it introduces

Explain why normal methods cannot solve those problems

Also need to introduce notation especially histories and then the
ouctome under competing histories.

Conclusion

    We denote the subject's history using an overhead bar
\(\bar A = \{1, 0, 1, 1\}\) and their individual time points as \(A_t\).
...
counterfactual. Also means saturated models are not an option because
they would be computationally intensive and so we use parametric models
which also links to positivity.
