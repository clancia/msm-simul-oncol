...
Estimating MSMs involves a parametric model for the weights and for
structural model relating exposure to outcome. To obtain unbiased
estimation of the causal effect of exposure on outcome correct model
specification is required for both models. Parametric models are
unlikely to be perfectly specified but should provide a good
...
Measurement error can arise in observational settings can arise in many
ways. Give example of bread,loaves and grams.

Second, all variables used in the analysis need to be correctly
measured. Measurement error in the treatment A, the outcome Y , or the
confounders L will generally result in bias (Chapter 9).

\begin{itemize}
\tightlist
\item
  measurement bias in causal structures \citet{Hernan2009}
\item
  similar to misscpecification we have no measurement error in
  simulations.
\item
  causal measures will differ from associational measures because the
  causal ones by definition have no mismeasurement.
\end{itemize}
...
simulation algorithm used to generate the data used in this thesis.
Survival analysis is the study of the distribution of life times. The
basic concepts reviewed here are a condensed version of those presented
in \citet{JohnP.Klein2003}.

\begin{itemize}
\tightlist
\item
  typically interested in effect of treatment on survival
\item
  Need to adjust for confounding variables and independent or other
  variables.
\end{itemize}
...
and a well defined end point. The survival function is the probability
that a certain individual survives until time \(t\) or equivalently the
probability that the survival time \(T\) is greater than \(t\),
\[S(t) = P(T > t)\].
...
In the discrete case, the hazard function is given
...
The shape of the hazard function depends on the context. In some cases
it can be an increasing function reflecting an increased chance of
failure over time. In other cases it can be increasing or hump shaped.
The shape of the hazard rate can be made to depend on
...
\item
  the hazard rate and hazard function are the same
\item
...
\item
  Explain what the hazard ratio is.
\item
...
Often interest lies in the relationships between In a typical study we
want to adjust for confounders and other variables in order to get a
better estimate than just looking at treated versus non-treated
patients.

\begin{itemize}
\tightlist
\item
  explain how we let the survival time dependent on covariates.
\item
...
it seems reasonable that the marginal effect (after marginalizing over
sex - pooling subjects) would also be equal to the male and female
specific effects. In other words, if the effect in males is the same as
in females then it is reasonable to think that the effect is the same in
the total population, which is made up of males and females. In that
case, the effect measure is collapsible because sex is unrelated to
exposure to ibuprofen and pooling males and females together and
collapsing over sex, correct analysis can be performed using a marginal
or naive analysis. This extends to the case of many subgroups defined
by, for example, age or race. Collapsibility is useful because it helps
to reduce the dimensionality and computational effort which arises when
many subgroups need to be taken account of in the analysis
\citet{Didelez2010}. \linebreak
...
\item
  need algorithm with this structure.
\end{itemize}
...
\item
  discuss \citet{Naimi2011} limitations of simulation algorithm.
\end{itemize}
...
\tightlist
\item
  stress that we still simulate from conditionals.
\end{itemize}
